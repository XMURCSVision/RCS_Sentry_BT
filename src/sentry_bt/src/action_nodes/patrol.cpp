...
Patrol::Patrol(const std::string &name, const BT::NodeConfiguration &config)
    : BT::ActionNodeBase(name, config), goal_handle_(nullptr),
      current_point_index_(0), is_navigating_(false) {
  ...
  RCLCPP_INFO(node_->get_logger(), "[Patrol]:已初始化 %zu 个巡逻点",
              patrol_points_.size());
}
...
void Patrol::sendNextGoal() {
  ...
  if (!client_->wait_for_action_server(std::chrono::seconds(1))) {
    RCLCPP_WARN(logger,
                "[Patrol]:未连接到navigate_to_pose服务，继续运行用于测试！");
    // 为了测试目的，模拟快速巡逻并返回成功
    ...
    if (patrol_points_.size() > 1) {
      ...
      RCLCPP_INFO(logger, "[Patrol]:随机选择下一个巡逻点 %zu",
  ...
  send_options.feedback_callback =
      std::bind(&Patrol::feedbackCallback, this, std::placeholders::_1,
                std::placeholders::_2);
