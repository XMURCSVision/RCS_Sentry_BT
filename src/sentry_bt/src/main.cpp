...
int main(int argc, char **argv) {
  ...
  // 参数默认值（可被 launch 命令行或 config/config.yaml 覆盖）
  ros_node->declare_parameter("enemy_lost_timeout_sec", 15.0);
  ...
  try {
    ...
    if (config["patrol_points"]) {
      ...
      RCLCPP_INFO(ros_node->get_logger(), "已加载 %zu 个巡逻点",
                  patrol_points.size());
    }
    if (config["supply_point"]) {
      ...
      RCLCPP_INFO(ros_node->get_logger(), "已设置补给点: x=%.2f y=%.2f",
                  supply_point.x, supply_point.y);
    }
    if (config["blood_threshold"]) {
      int blood_threshold = config["blood_threshold"].as<int>();
      blackboard->setBloodThreshold(blood_threshold);
      RCLCPP_INFO(ros_node->get_logger(), "已设置血量阈值: %d",
                  blood_threshold);
    }
    if (config["blood"]) {
    ...
    if (config["ammo_full"]) {
      int ammo_full = config["ammo_full"].as<int>();
      blackboard->setFullAmmoThreshold(ammo_full);
      RCLCPP_INFO(ros_node->get_logger(), "已设置满弹量: %d", ammo_full);
    }
    if (config["ammo_threshold"]) {
      int ammo_threshold = config["ammo_threshold"].as<int>();
      blackboard->setAmmoThreshold(ammo_threshold);
      RCLCPP_INFO(ros_node->get_logger(), "已设置低弹阈值: %d", ammo_threshold);
    }
    if (config["ammo"]) {
      int ammo = config["ammo"].as<int>();
      blackboard->setAmmo(ammo);
      RCLCPP_INFO(ros_node->get_logger(), "已设置弹量: %d", ammo);
    }

    if (config["enemy_lost_timeout_sec"]) {
      double sec = config["enemy_lost_timeout_sec"].as<double>();
      ros_node->set_parameter(rclcpp::Parameter("enemy_lost_timeout_sec", sec));
      RCLCPP_INFO(ros_node->get_logger(), "已设置 enemy_lost_timeout_sec: %.2f",
                  sec);
    }
  ...
  factory.registerNodeType<sentry_bt::AttackModeSelector>("AttackModeSelector");
  factory.registerNodeType<sentry_bt::IsTacticalMode>("IsTacticalMode");
  factory.registerNodeType<sentry_bt::IsAttackMode>("IsAttackMode");
  ...
  TacticalStateUpdater tactical_updater(ros_node, blackboard);

  rclcpp::Rate rate(10);
  while (rclcpp::ok()) {
    rclcpp::spin_some(ros_node);
    tactical_updater.update(ros_node->get_clock()->now());
    tree.tickRoot();
    ros.publish();
    rate.sleep();
  }
